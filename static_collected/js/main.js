...
(function () {
  const grid = document.getElementById('grid-steel-mill-new-card-with-animation');
  if (!grid) return;
  const cards = grid.querySelectorAll('.card-steel-mill-new-card-with-animation');

  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('show-steel-mill-new-card-with-animation');
      } else {
        // remove so it replays when scrolling back (LIFO feel)
        e.target.classList.remove('show-steel-mill-new-card-with-animation');
      }
    });
  }, { threshold: 0.18 });
...
(function () {
  const items = document.querySelectorAll('.reveal-steel-mill-new-panel-');
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-steel-mill-new-panel-');
  ...
  const form = document.getElementById('service-form-steel-mill-new-panel-');
...
(() => {
  const els = document.querySelectorAll('.reveal-steel-mill-new-panel-');
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-steel-mill-new-panel-');
...
(() => {
  const items = document.querySelectorAll('.reveal-steel-mill-new-about-');
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-steel-mill-new-about-');
        io.unobserve(e.target);
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  const els = document.querySelectorAll(
    '.reveal-left-steel-mill-new-services, .reveal-right-steel-mill-new-services, .reveal-up-steel-mill-new-services'
  );
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-steel-mill-new-services');
        io.unobserve(e.target);
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  const els = document.querySelectorAll(
    '.reveal-left-steel-mill-new-process, .reveal-right-steel-mill-new-process'
  );
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-steel-mill-new-process');
        io.unobserve(e.target);
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  const targets = document.querySelectorAll(
    '.reveal-left-steel-mill-new-why-us, .reveal-right-steel-mill-new-why-us, .reveal-top-steel-mill-new-why-us, .reveal-bottom-steel-mill-new-why-us'
  );
  ...
  const revealOnScroll = () => {
    targets.forEach(target => {
      ...
      if (inView) {
        target.classList.add('reveal-in-steel-mill-new-why-us');
      } else {
        target.classList.remove('reveal-in-steel-mill-new-why-us');
      }
    });
  };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const detailsMap = {
   hotRollingMill: {
  title: 'Hot Rolling Mill Systems for Bars, Rods, Sections, and Flats',
  body: `
    <p><strong>At iEngineering Australia</strong>, we deliver high-capacity <strong>Hot Rolling Mill Systems</strong> engineered to convert <strong>billets, blooms, and slabs</strong> into finished products such as <strong>bars, rods, sections, and flats</strong>. Our solutions are designed for <strong>high throughput</strong>, <strong>dimensional accuracy</strong>, and <strong>consistent metallurgical performance</strong>, supported by robust mechanical design and reliable automation.</p>

    <h4>Key Features</h4>
    <ul>
      <li><strong>High Throughput Operation:</strong> Optimised rolling sequences and equipment selection for continuous production.</li>
      <li><strong>Dimensional Accuracy:</strong> Consistent product tolerances achieved through stable mill setup and control.</li>
      <li><strong>Metallurgical Consistency:</strong> Controlled processing to support uniform material properties and quality.</li>
      <li><strong>Robust Mechanical Design:</strong> Heavy-duty components suitable for high loads and harsh mill environments.</li>
      <li><strong>Automation Support:</strong> Practical control and monitoring architecture for safe and repeatable operation.</li>
    </ul>

    <h4>System Scope</h4>
    <p>We support complete or partial hot rolling mill packages, including:</p>
    <ul>
      <li><strong>Process and layout engineering:</strong> Mill arrangement, material flow, and throughput optimisation.</li>
      <li><strong>Mechanical packages:</strong> Stands, guides, roller tables, drives integration, and auxiliary systems.</li>
      <li><strong>Electrical and automation:</strong> MCCs, control panels, instrumentation, PLC/SCADA integration, and interlocks.</li>
      <li><strong>Commissioning support:</strong> Site testing, tuning, and documentation handover.</li>
    </ul>

    <h4>Applications</h4>
    <ul>
      <li>Steel plants and rolling mills</li>
      <li>Rebar and wire rod production lines</li>
      <li>Structural sections and flats manufacturing</li>
      <li>Capacity upgrades and retrofit projects</li>
    </ul>
  `,
  // image: { src: '../static/images/solutions/hot-rolling-mill.avif', alt: 'Hot rolling mill systems' }
},
coldRollingMill: {
  title: 'Cold Rolling Mill Systems for Sheets, Strips, and Coils',
  body: `
    <p><strong>At iEngineering Australia</strong>, we provide precision <strong>Cold Rolling Mill Systems</strong> designed to produce high-quality <strong>sheets, strips, and coils</strong> with <strong>excellent surface finish</strong>, <strong>tight tolerances</strong>, and <strong>enhanced mechanical properties</strong>. Our solutions are ideal for industries that demand consistent quality and repeatable production outcomes.</p>

    <h4>Key Benefits</h4>
    <ul>
      <li><strong>Tight Dimensional Tolerances:</strong> Stable thickness control for consistent product quality.</li>
      <li><strong>Superior Surface Finish:</strong> Controlled rolling parameters to achieve clean, uniform surfaces.</li>
      <li><strong>Enhanced Mechanical Properties:</strong> Improved strength and formability through precise reduction control.</li>
      <li><strong>High Repeatability:</strong> Reliable operation for long runs and multi-grade production.</li>
      <li><strong>Automation Support:</strong> Practical control, monitoring, and interlocks for safe operation.</li>
    </ul>

    <h4>System Scope</h4>
    <p>We support complete or partial cold rolling mill packages, including:</p>
    <ul>
      <li><strong>Process and layout engineering:</strong> Material flow, line configuration, and productivity optimisation.</li>
      <li><strong>Mechanical integration:</strong> Mill stand integration, strip handling, and auxiliary systems support.</li>
      <li><strong>Electrical and controls:</strong> MCCs, control panels, instrumentation, PLC/SCADA integration, and safety interlocks.</li>
      <li><strong>Commissioning support:</strong> Site testing, tuning, and documentation handover.</li>
    </ul>

    <h4>Applications</h4>
    <ul>
      <li>Automotive steel and component manufacturing</li>
      <li>Appliances and white goods production</li>
      <li>High-quality sheet and strip processing</li>
      <li>Precision steel and specialty metal applications</li>
    </ul>
  `
  // image: { src: '../static/images/solutions/cold-rolling-mill.avif', alt: 'Cold rolling mill systems' }
},

    sectionStructuralRollingMill: {
  title: 'Section & Structural Rolling Mill Systems for Construction and Infrastructure',
  body: `
    <p><strong>At iEngineering Australia</strong>, we deliver robust <strong>Section & Structural Rolling Mill Systems</strong> designed for the production of <strong>angles, channels, beams, and structural sections</strong> used across construction, infrastructure, and heavy engineering industries. Our solutions are engineered for <strong>dimensional accuracy</strong>, <strong>structural integrity</strong>, and <strong>reliable high-load operation</strong>.</p>

    <h4>Key Features</h4>
    <ul>
      <li><strong>Accurate Section Geometry:</strong> Consistent profiles and dimensional control for structural applications.</li>
      <li><strong>High Mechanical Strength:</strong> Rolling solutions designed to maintain required load-bearing properties.</li>
      <li><strong>Heavy-Duty Design:</strong> Robust mill stands and components suitable for high rolling forces.</li>
      <li><strong>Production Flexibility:</strong> Capability to manufacture multiple section sizes and profiles.</li>
      <li><strong>Automation & Control:</strong> Integrated control and monitoring systems for stable and safe operation.</li>
    </ul>

    <h4>System Scope</h4>
    <p>We support complete or partial section and structural rolling mill packages, including:</p>
    <ul>
      <li><strong>Process and layout engineering:</strong> Mill configuration, pass design support, and material flow optimisation.</li>
      <li><strong>Mechanical systems:</strong> Section mill stands, guides, roller tables, and auxiliary equipment integration.</li>
      <li><strong>Electrical and automation:</strong> MCCs, control panels, PLC/SCADA integration, drives, and safety interlocks.</li>
      <li><strong>Commissioning & upgrades:</strong> New installations, capacity expansions, and retrofit projects.</li>
    </ul>

    <h4>Applications</h4>
    <ul>
      <li>Construction and infrastructure projects</li>
      <li>Bridges, buildings, and industrial structures</li>
      <li>Heavy engineering and fabrication industries</li>
      <li>Rolling mill modernisation and capacity upgrades</li>
    </ul>
  `
  // image: { src: '../static/images/solutions/section-structural-rolling-mill.avif', alt: 'Section and structural rolling mill systems' }
},
auxiliarySteelMillSystems: {
  title: 'Auxiliary Steel Mill Systems for Reliable and Efficient Operations',
  body: `
    <p><strong>At iEngineering Australia</strong>, we provide a comprehensive range of <strong>Auxiliary Steel Mill Systems</strong> that support and enhance the performance of primary rolling and processing lines. Our auxiliary solutions are engineered to improve <strong>operational reliability</strong>, <strong>process efficiency</strong>, and <strong>plant safety</strong> across steel manufacturing facilities.</p>

    <h4>Key Functions</h4>
    <ul>
      <li><strong>Material Handling Support:</strong> Reliable transfer of billets, slabs, coils, and finished products.</li>
      <li><strong>Process Stability:</strong> Supporting systems that ensure smooth and continuous mill operation.</li>
      <li><strong>Operational Safety:</strong> Designed with appropriate interlocks and protection systems.</li>
      <li><strong>High Availability:</strong> Engineered to minimise downtime and support continuous production.</li>
      <li><strong>Automation Integration:</strong> Seamless integration with plant control and monitoring systems.</li>
    </ul>

    <h4>System Scope</h4>
    <p>Our auxiliary steel mill solutions include, but are not limited to:</p>
    <ul>
      <li><strong>Roller tables and conveyors:</strong> For efficient material transport within the mill.</li>
      <li><strong>Cooling beds and run-out tables:</strong> Controlled cooling and product handling.</li>
      <li><strong>Shearing and cutting systems:</strong> Accurate cutting to length and downstream processing support.</li>
      <li><strong>Hydraulic and pneumatic systems:</strong> Reliable actuation for mill operations.</li>
      <li><strong>Utilities and services:</strong> Power distribution, instrumentation, and control panels.</li>
    </ul>

    <h4>Applications</h4>
    <ul>
      <li>Hot and cold rolling mills</li>
      <li>Section and structural rolling lines</li>
      <li>Steel processing and finishing plants</li>
      <li>Modernisation and plant expansion projects</li>
    </ul>
  `
  // image: { src: '../static/images/solutions/auxiliary-steel-mill-systems.avif', alt: 'Auxiliary steel mill systems' }
}
  ...
  grid.addEventListener('click', (e) => {
    const btn = e.target.closest('.svc-cta-steel-mill-new-services');
    if (!btn) return;
  ...
  if (exploreBtn) {
    const allCards = Array.from(grid.querySelectorAll('.svc-item-steel-mill-new-services'));
    ...
    function setExpanded(state) {
      ...
      extraCards.forEach(el => {
        el.classList.remove('reveal-in-steel-mill-new-services');
        void el.offsetWidth;
      });
    }
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('reveal-in-steel-mill-new-services');
      } else {
        entry.target.classList.remove('reveal-in-steel-mill-new-services');
      }
    });
...
(() => {
  const els = document.querySelectorAll('.reveal-steel-mill-new-get-in-touch');
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      el.querySelectorAll?.('.stat-num-steel-mill-new-get-in-touch').forEach(counter => {
