...
(function () {
  ...
  function showToast(msg, type = "error", timeoutMs = 4000) {
    ...
    el.className = "steel-mill-new-toast " + (type === "ok" ? "steel-mill-new-toast--ok" : "steel-mill-new-toast--error");
    el.innerHTML = `<span aria-hidden="true">${type === "ok" ? "✔️" : "⚠️"}</span>
                    <div>${msg}</div>
                    <button class="steel-mill-new-toast__close" aria-label="Close">×</button>`;
    root.appendChild(el);

    const remove = () => el.remove();
    el.querySelector(".steel-mill-new-toast__close").addEventListener("click", remove);
